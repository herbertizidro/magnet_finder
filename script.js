...
document.addEventListener('DOMContentLoaded', function(){
	...
	document.querySelector('#btn').addEventListener('click', function(){
		...
		document.querySelector('#status-busca').innerHTML = '<div class="d-flex justify-content-center"><div class="spinner-border" style="color: #FF1616; width: 2rem; height: 2rem;" role="status"><span class="sr-only">Loading...</span></div></div>';
		let pesquisa = document.querySelector('#termo-pesquisa').value;
		try{
			...
			if(pesquisa.length == 0){
				...
				let url = `https://api.sumanjay.cf/torrent/?query=${pesquisa}`;							
